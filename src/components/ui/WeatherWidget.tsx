...
const getWeatherIcon = (code: number, isDay: boolean = true) => {
    ...
    if (code === 1000) {
        return isDay
            ? <Sun size={30} className="text-amber-500 dark:text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.4)]" />
            : <Moon size={30} className="text-indigo-600 dark:text-indigo-200 drop-shadow-[0_0_8px_rgba(199,210,254,0.3)]" />;
    }

    // Part cloudy (1003)
    if (code === 1003) return <Cloud size={30} className={cn(isDay ? "text-sky-700 dark:text-sky-300" : "text-slate-600 dark:text-slate-400")} />;

    // Cloudy / Overcast (1006, 1009)
    if (code === 1006 || code === 1009) return <Cloud size={30} className="text-slate-600 dark:text-slate-400" />;

    // Mist, Fog (1030, 1135, 1147)
    if (code === 1030 || code === 1135 || code === 1147) return <CloudFog size={30} className="text-slate-500 dark:text-slate-300/80" />;

    // Patchy rain, light drizzle (1063, 1150, 1153, 1180, 1183)
    if ([1063, 1150, 1153, 1180, 1183].includes(code)) return <CloudDrizzle size={30} className="text-cyan-700 dark:text-cyan-400" />;

    // Moderate/Heavy Rain (1186, 1189, 1192, 1195, 1240, 1243, 1246)
    if ([1186, 1189, 1192, 1195, 1240, 1243, 1246].includes(code)) {
        return <CloudRain size={30} className="text-blue-700 dark:text-blue-500 drop-shadow-[0_0_5px_rgba(59,130,246,0.3)]" />;
    }

    // Snow (1066, 1114, 1117, 1210-1225, 1255-1258)
    if (code >= 1210 && code <= 1258 || [1066, 1114, 1117].includes(code)) return <CloudSnow size={30} className="text-sky-800 dark:text-blue-50" />;

    // Thundery outbreaks (1087, 1273, 1276, 1279, 1282)
    if ([1087, 1273, 1276, 1279, 1282].includes(code)) {
        return <CloudLightning size={30} className="text-amber-600 dark:text-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.5)]" />;
    }

    return <Wind size={30} className="text-teal-700 dark:text-teal-300" />;
};
