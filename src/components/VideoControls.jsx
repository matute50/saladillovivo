...
const VideoControls = ({
  ...
  const handleVolumeChangeWrapper = (value) => {
    onVolumeChange(value);
  };

  const VolumeIcon = isMuted || volume === 0 ? VolumeX : volume < 0.5 ? Volume1 : Volume2;
  const volumeIconColorClass = "text-white";
    ...
    >
      ...
      <div className="flex items-center justify-between space-x-2 p-2">
        <div className="flex items-center space-x-2">
          <button onClick={(e) => handleInteraction(e, togglePlayPause)} className="text-white p-1 focus:outline-none control-button">
          ...
          >
            <button onClick={(e) => handleInteraction(e, toggleMute)} className={`${volumeIconColorClass} p-1 focus:outline-none transition-colors`}>
