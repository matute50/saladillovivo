...
const NewsCard: React.FC<NewsCardProps> = ({ newsItem, index = 0, className = '', isFeatured = false }) => {
  ...
  return (
    <motion.article
      ...
      className={cn(
        "relative flex flex-col rounded-xl overflow-hidden transition-all duration-300 h-full",
    ...
    >
      <div className="relative w-full h-full aspect-video overflow-hidden bg-black">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 pr-12">
            <h3 className={`font-bold ${titleSizeClass} text-white leading-tight drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] transition-colors line-clamp-3`}>
              {title}
            </h3>
        </div>
        ...
        {isPlayable && (
            <div className={cn(
                "absolute bottom-2 right-2 flex items-center justify-center rounded-full p-0.5 border border-white shadow-lg shadow-black/50 backdrop-blur-md bg-black/40 hover:bg-[#003399] hover:bg-opacity-100",
