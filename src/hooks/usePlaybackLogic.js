...
const useFader = (initialVolume = 1.0) => {
  ...
  const ramp = useCallback((targetVolume, duration, onComplete) => {
...
export const usePlaybackLogic = ({
  ...
  const performTransition = useCallback((callback) => {
  ...
  const playMedia = useCallback((mediaData, isAutoPlay = false) => {
  ...
  const playUserSelectedVideo = useCallback((video, categoryName) => {
    const mediaData = {
      url: video.url,
      title: video.nombre,
      type: 'video',
